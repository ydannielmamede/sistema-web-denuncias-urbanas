...
const accounts = [
  {
    name: "Melissa",
    email: "melissa@administracao.com",
    role: "Admin",
    status: "Ativo",
    joined: "Jan 2024",
    av: "S",
  ...
  {
    name: "Daniel",
    email: "daniel@administracao.com",
    role: "Operador",
    status: "Ativo",
    joined: "Abr 2024",
    av: "C",
  ...
  {
    name: "Romulo",
    email: "romulo@administracao.com",
    role: "Operador",
    status: "Ativo",
    joined: "Jul 2024",
    av: "D",
...
const updatesData = [
  {
    ...
    desc: "Versão 2.3.1 instalada com sucesso.",
    time: "Agora",
    sc: "green",
  },
  {
    ...
    desc: "Denúncia #808 - Esgoto transbordando na Vila Nova.",
    time: "5 min",
    sc: "blue",
  },
  {
    ...
    desc: "Taxa de denúncias 35% acima da média.",
    time: "22 min",
    sc: "yellow",
  },
  {
    type: "success",
    title: "Denúncia #803 resolvida",
    desc: "Árvore caída na Av. Central foi removida.",
    time: "1h",
    sc: "green",
  },
  {
    ...
    desc: "Relatório mensal de abril disponível para download.",
    time: "3h",
    sc: "purple",
  },
  {
    ...
    desc: "Manutenção preventiva em 17/04 às 02:00.",
    time: "1 dia",
    sc: "blue",
  },
  {
    ...
    desc: "Backup dos dados realizado com sucesso.",
    time: "2 dias",
    sc: "green",
  },
];
...
function syncThemeToggle() {
  ...
  const dashboardIcon = document.getElementById("dashboardThemeIcon");

  if (st) {
    isDark ? st.classList.add("on") : st.classList.remove("on");
  }

  if (dashboardToggle) {
    dashboardToggle.setAttribute("aria-pressed", isDark);
  }

  if (dashboardIcon) {
    dashboardIcon.innerHTML = isDark
...
document.addEventListener("componentsLoaded", syncThemeToggle);
document
  .getElementById("dashboardThemeToggle")
  .addEventListener("click", () => applyDashboardTheme(!isDark));
document
...
const badgeIcons = {
  ...
  green:
    '<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>',
};
...
function renderRow(r, showPrio) {
  ...
  const prioCell = showPrio
    ? `<td class="td-muted" style="color:${r.prioColor};font-weight:600">${r.prio}</td>`
    : "";
  return `<tr>
...
function renderDashTable() {
  ...
  document.getElementById("dashTableBody").innerHTML = filtered
    .map((r) => renderRow(r, false))
    .join("");
  document.getElementById("dashTableInfo").textContent =
    `Mostrando ${filtered.length} de ${reports.length} resultados`;
}
...
document.getElementById("dashFilterTabs").addEventListener("click", (e) => {
  ...
  document
    .querySelectorAll("#dashFilterTabs .filter-tab")
...
function renderReportsTable(q) {
  const list = q
    ...
    : reports;
  document.getElementById("reportsTableBody").innerHTML = list
...
document
  ...
  .addEventListener("input", (e) =>
    renderReportsTable(e.target.value.toLowerCase()),
  );
renderReportsTable("");
...
function renderAccounts(q) {
  const list = q
    ...
    : accounts;
  document.getElementById("accountsList").innerHTML = list
    .map(
      (a) => `
...
document
  .getElementById("accountsSearch")
  .addEventListener("input", (e) =>
    renderAccounts(e.target.value.toLowerCase()),
  );
renderAccounts("");
...
document.getElementById("updatesList").innerHTML = updatesData
  .map(
    (u) => `
...
document.getElementById("categoriesChart").innerHTML = cats
  .map(
    (c) => `
...
(function() {
  ...
  const minV = 0,
    maxV = Math.ceil(Math.max(...allVals) / 20) * 20;
  ...
  const makePath = (key) => {
    ...
    data.forEach((p, i) => {
      ...
      if (i === 0) d += `${x},${y}`;
        ...
        d += ` C${cx},${py(data[i - 1][key])} ${cx},${y} ${x},${y}`;
      }
    });
    return d;
  };
...
(function() {
  ...
  data.forEach((p, i) => {
    ...
    if (i === 0) path += `${x},${y}`;
      ...
      const cx = (px(i) + px(i - 1)) / 2;
      path += ` C${cx},${py(data[i - 1].v)} ${cx},${y} ${x},${y}`;
    }
